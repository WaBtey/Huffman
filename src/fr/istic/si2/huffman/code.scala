...
object ConstructionCode {
   ...
   * @param total le nombre total de chars dans le texte
   * @return la fréquence d'apparition de chaque chars
   */
  def frequencesChars (l: List[Char], total:Int): List[(Char, Double)] = {
    l match {
      case Nil => Nil
      case head :: tail => compteEtSupprime(head, l) match {
        case (count, reste) => (head, count/total.toDouble) :: frequencesChars(reste, total)
      }
    }
  }
  ...
  def analyseFrequences(s: String): List[(Char, Double)] = frequencesChars(s.toList, s.length)
